...
* **操作系统**: Linux / Ubuntu (推荐带 XFCE 等桌面环境以运行 Chromium)
* **Python 版本**: Python 3.10+
* **关键依赖**: SQLite3 (支持 WAL 模式), Redis (推荐), Chromium 浏览器
...
### 第三步：启动 Host Chromium CDP 远程调试

Finance Subagent 需要连接一个**常驻的主机 Chromium 进程**，以便共享 Session Cookie 绕过验证码。

1. **在终端后台启动 Chromium 浏览器**:
   ```bash
   chromium-browser --remote-debugging-port=9222 --user-data-dir=/home/remora/.config/chromium-hermes --no-first-run --disable-gpu --window-size=1280,720 &
   ```
